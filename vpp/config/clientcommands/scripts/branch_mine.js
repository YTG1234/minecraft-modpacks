var wantedBlocks = ["diamond_ore", "iron_ore"];
...
var isWantedItemEntity = function (entity) {
    ...
    var nbt = entity.nbt;
    if (!nbt.Item) return false;
    var type = nbt.Item.id;
    for (var i = 0; i < wantedBlocks.length; i++)
        if (type === "minecraft:" + wantedBlocks[i]) return true;
    return (
...
var canWalkOn = function (block) {
    ...
    if (BlockState.defaultState(block).liquid) return false;
    return true;
};
...
var getTool = function (block) {
    var effective = function (item, block) {
        ...
        if (
            !BlockState.defaultState(block).canBreakByHand &&
            !stack.isEffectiveOn(block)
        )
            return false;
        return stack.getMiningSpeed(block) > 1;
    };
    if (effective("diamond_pickaxe", block)) return "pickaxe";
    if (effective("diamond_shovel", block)) return "shovel";
    if (effective("diamond_axe", block)) return "axe";
    if (effective("diamond_sword", block)) return "sword";
    return null;
};
...
var centerPlayer = function () {
    ...
    while (
        ...
        tick();
    if (
        Math.abs(Math.floor(player.x) + 0.5 - player.x) >= 0.2 ||
        Math.abs(Math.floor(player.z) + 0.5 - player.z) >= 0.2
    )
        return player.moveTo(
            Math.floor(player.x) + 0.5,
            Math.floor(player.z) + 0.5
        );
    return true;
};
...
var mineBlock = function (x, y, z) {
    ...
    if (stateAbove.fallable) {
        while (world.getBlock(x, y, z) !== stateAbove.block) tick();
        if (!mineBlock(x, y, z)) throw new Error();
    }
...
var makeBridge = function (x, y, z, dx, dz) {
    ...
    while (Math.floor(player.x) === x && Math.floor(player.z) === z) {
        if (!continueSneaking()) throw new Error();
    }
...
var mineNearbyOre = function (x, y, z) {
    ...
    for (var dy = 1; dy <= 2; dy++) {
        if (canWalkThrough(world.getBlock(x, y + dy, z))) {
            for (var dir = 0; dir < cardinals6.length; dir++) {
                var ddx = cardinals6[dir][0],
                    ddy = cardinals6[dir][1],
                    ddz = cardinals6[dir][2];
                if (
                    isWantedBlock(
    ...
    // step up
    for (var i = 0; i < 4; i++) {
        var dx = cardinals4[i][0],
            dz = cardinals4[i][1];

        // check if we want to step up
        if (!canWalkThrough(world.getBlock(x + dx, y + 2, z + dz))) continue;
        if (
            !canWalkThrough(world.getBlock(x + dx, y + 1, z + dz)) &&
            !canWalkThrough(world.getBlock(x, y + 2, z))
        )
            continue;
        var wantToStepUp = false;
        for (var j = 0; j < 6; j++) {
            var ddx = cardinals6[j][0],
                ddy = cardinals6[j][1],
                ddz = cardinals6[j][2];
            if (
                isWantedBlock(
                    world.getBlock(x + dx + ddx, y + 2 + ddy, z + dz + ddz)
                )
            ) {
                wantToStepUp = true;
                break;
        ...
        if (!canWalkThrough(world.getBlock(x + dx, y + 1, z + dz)))
            if (!mineBlock(x + dx, y + 1, z + dz)) throw new Error();
        if (!canWalkThrough(world.getBlock(x, y + 2, z)))
            if (!mineBlock(x, y + 2, z)) throw new Error();
        if (!canWalkOn(world.getBlock(x + dx, y, z + dz)))
            if (!placeBlock(x + dx, y, z + dz)) continue;
        ...
        for (var dy = 2; dy >= 0; dy--) {
            if (!canWalkThrough(world.getBlock(x, y + dy, z)))
                if (!mineBlock(x, y + dy, z)) throw new Error();
        }
    ...
    for (var i = 0; i < 4; i++) {
        ...
        if (isWantedBlock(world.getBlock(x + dx, y, z + dz))) {
            if (!mineBlock(x + dx, y, z + dz)) throw new Error();

            if (!canWalkOn(world.getBlock(x + dx, y - 1, z + dz)))
                if (!placeBlock(x + dx, y - 1, z + dz)) continue;
            ...
            if (canWalkThrough(world.getBlock(x + dx, y + dy, z + dz))) {
                for (var j = 0; j < 6; j++) {
                    ...
                    if (
                        ...
                        if (!clearWay(x, y, z, dx, dz)) throw new Error();

                        centerPlayer();
                        if (!player.moveTo(x + dx + 0.5, z + dz + 0.5))
                            throw new Error();
                        if (!mineNearbyOre(x + dx, y, z + dz))
                            throw new Error();
                        centerPlayer();
                        for (var dy = 1; dy >= 0; dy--) {
                            if (!canWalkThrough(world.getBlock(x, y + dy, z)))
    ...
    for (var i = 0; i < 4; i++) {
        ...
        if (
            canWalkThrough(world.getBlock(x + dx, y, z + dz)) &&
            isWantedBlock(world.getBlock(x + dx, y - 1, z + dz))
        ) {
            if (!mineBlock(x + dx, y - 1, z + dz)) throw new Error();
            if (!canWalkOn(world.getBlock(x + dx, y - 2, z + dz)))
                if (!placeBlock(x + dx, y - 2, z + dz)) continue;
...
var makeTunnel = function (x, y, z, dx, dz) {
    ...
    // walk to next spot
    if (!player.moveTo(x + dx + 0.5, z + dz + 0.5, false)) throw new Error();

    // place torch
    if (world.getBlockLight(x, y, z) <= 1) {
        if (!player.pick("torch")) throw new Error();
...
var pickUpItemsLoop = function () {
    while (true) {
        ...
        for (var i = 0; i < items.length; i++) {
            var item = items[i];
            if (
                Math.abs(item.x - player.x) < 1.425 &&
                Math.abs(item.z - player.z) < 1.425 &&
                item.y - player.y >= -0.5 &&
                item.y - player.y <
                    2.3 - player.standingEyeHeight + player.eyeHeight
            ) {
                if (isWantedItemEntity(item)) {
                    ...
                    if (
                        nbt.Item &&
                        itemsWanted.indexOf((itemId = nbt.Item.id)) === -1
                    )
        ...
        for (var slot = 0; slot < 36; slot++) {
            if (
                playerItems[slot].Count < 64 &&
                itemsWanted.indexOf(playerItems[slot].id) !== -1
            )
                itemsWanted.splice(
                    itemsWanted.indexOf(playerItems[slot].id),
                    1
                );
            if (
                playerItems[slot].id === "minecraft:cobblestone" ||
                playerItems[slot].id === "minecraft:stone"
            )
                cobblestoneCount++;
        }
        for (var slot = 0; slot < 36 && itemsWanted.length > 0; slot++) {
            if (playerItems[slot].id === "minecraft:air") itemsWanted.pop();
        }
        for (var slot = 0; slot < 36 && itemsWanted.length > 0; slot++) {
